...
export const  ModalDelete: React.FC<Props> = ({fetchallposts}) => {
  ...
  const onClickOk = async () => {
    if (!delStore.delID) return
    delPost(delStore.delID)
    delStore.setdelID(null)
    delStore.setOpenModal(false)
  }
  ...
  return (
    <Dialog open={delStore.isOpenModal} onClose={()=>delStore.setOpenModal(false)} className="relative z-10">
      <DialogBackdrop
        transition
                ...
                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Статья будет удалена безвозвратно.<br></br>Вы действительно желаете удалить эту статью ?
                    </p>
                  </div>
                </div>
              </div>
