...
const FilterMenu: React.FC<PropsType> = ({ data }) => {
  ...
  return (
    <Menu as="div" className="inline-block text-left select-none">
      <div>
        <MenuButton className="inline-flex w-44 justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
          {postsStore.selectCategory ? postsStore.selectCategory : "Все"}
          <ChevronDownIcon
            aria-hidden="true"
      ...
      >
        ...
        <div className="py-1">
          <MenuItem>
            <a
              onClick={() => postsStore.setSelectCategory(null)}
          ...
          {data.map((i) => (
            <MenuItem key={i}>
              <a
                onClick={() => postsStore.setSelectCategory(i)}
                className="block px-4 py-2 text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:text-gray-900"
              >
                {i}
              </a>
