...
const Page:React.FC = () => {
    ...
    useEffect(()=>{
        axios.get("http://localhost:5000/didbuyitems")
          .then((res) => setdidBuy(res.data))
  
        axios.get("http://localhost:5000/moneyitems")
    ...
    axios.get("http://localhost:5000/buyitem/"+e.currentTarget.id)
      .then((res) => {
    ...
    <div className='text-center grid grid-cols-1 lg:grid-cols-2 w-auto mx-5'>
        <div className="">
                ...
                <div className="w-full select-none">
                    {
                        moneyitems.map((i, index)=>{
                            return (
                                <ItemMoney key={index} index={index} img={i.img} name={i.name} amount={i.amount} actions={{buy: buyMoney, del: delMoneyItem}}/>
                            )
                        })
                    }
            ...
            <div className='pl-32 text-gray-700 m-2 mx-10 text-right '>
                <p className='text-4xl font-bold'>Мне нужны денежки, чтобы купить это</p>
        ...
        <p>Я уже купил</p>
            <p className='text-gray-700 mx-auto m-8 text-sm'>
                                ...
                                {didBuy.map((i, index)=>(
                                    <DidBuy key={index} item={i} focus={{id: focusTask, set_focus: setFocusTask}}/>
                                ))}
            </p>
        </div>
    </div>
